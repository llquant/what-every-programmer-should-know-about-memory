...
这个信息将所有的一切连系在一起；现在我们有个机器架构的完整轮廓了。我们已经知道机器拥有四个处理器。每个处理器构成它自己的节点，可以从 `node*` 目录的 `cpumap` 文件中的值里头设置的 bit 看出来。在那些目录的 `distance` 文件包含一组值，一个值代表一个节点，表示在各个节点上访问内存的成本。在这个例子中，所有本地内存访问的成本为 10，所有对任何其他节点的远程访问的成本为 20。[^26]这表示，即使处理器被组织成一个二维超立方体（见图 5.1），在没有直接连接的处理器之间访问也没有比较贵。成本的相对值应该能用来作为访问时间的实际差距的估计。所有这些信息的准确性是另一个问题了。
