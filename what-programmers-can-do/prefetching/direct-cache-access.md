...
在现代操作系统中，cache 错失的一个来源是到来的数据流量的处理。像网卡（Network Interface Card，NIC）与硬盘控制器等现代硬件，可以在不涉及 CPU 的情况下，直接将接收或读取的数据写入到 memory 中。这对于我们如今拥有的装置的性能而言至关重要，但它也造成问题。假使有个从网络传入的封包：操作系统必须检查封包的头（header）以决定要如何处理它。NIC 将封包摆进 memory，然后通知处理器它的到来。处理器没有机会去预取数据，因为它并不知道数据将何时抵达，甚至可能不会确切知道它将会被存在哪。结果是在读取头时的一次 cache 错失。
...
在第二步中，传统的行为仅会是以链接到 memory 的连线完成 DMA 传输。对于被设置 DCA 标志的 DMA 传输，北桥会以特殊的、新的 DCA 标志在 FSB 上同时送出数据。处理器一直窥探着 FSB，并且若是它认出 DCA 标志，它会试着将寄给处理器的数据加载到最低阶 cache 中。事实上，DCA 标志是个提示；处理器可以自由地忽略它。在 DMA 传输完成之后，会以信号通知处理器。

在处理封包时，操作系统必须先确定是哪种封包。若是 DCA 提示没有被忽略的话，操作系统必须执行、以识别封包的加载操作很有可能会命中 cache。将每个封包数以百计个循环的节约，乘上每秒能处理的成千上万个封包，节省的加总量是个非常可观的数字，尤其在谈到等待时间的时候。
